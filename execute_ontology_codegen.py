#!/usr/bin/env python3
"""
Execute ontology code generation strand.

CRITICAL - Generate deterministic Python code to transform YAML sources into
ontology JSON, eliminating LLM transcription errors.
"""
...
async def main():
    print("⚙️  Ontology Code Generation Strand")
    print("=" * 80)
    print("Principle: Code-generated data > LLM-interpreted data")
    print("Goal: Deterministic YAML → JSON transformation")
    print("=" * 80)
    print()
    ...
    seed_path = base_dir / 'SEED_ontology_codegen.json'
    ...
    context = {
        ...
        "goal": seed['goal'],
        "current_issue": seed['context']['current_issue'],
        "architectural_principle": seed['context']['architectural_principle'],
        "source_files": seed['context']['source_files'],
        "additional_data": seed['context']['additional_data'],
        "phases": seed['phases'],
    ...
    print("Creating code generation strand in fork space...")
    print()
    ...
    print()
    print("Executing 3-agent code generation workflow:")
    print("  Phase 1: Architecture design (architect)")
    print("  Phase 2: Implementation (implementer)")
    print("  Phase 3: Validation synthesis (coordinator)")
    print()
    print("Output: ontology/generate_ontology.py (deterministic script)")
    print()
    ...
    print()
    print("=" * 80)
    print("🎉 ONTOLOGY CODE GENERATION COMPLETE")
    print("=" * 80)
    ...
    result_file = strand_results_dir / f'STRAND_ontology_codegen_{strand.definition.strand_id[:8]}.json'
    result_data = {
    ...
    print("📦 Extracting code generation artifacts:")
        ...
        if isinstance(result.findings, dict):
            for agent_name, content in result.findings.items():
                if content and agent_name not in ['_hemn_space', '_token_usage']:
                    artifact_file = strand_results_dir / f'CODEGEN_{agent_name}.md'
    ...
    print()
    print("📋 Deliverables:")
    print("  ✓ ontology/generate_ontology.py (deterministic script)")
    print("  ✓ ontology/HD_ONTOLOGY_complete.json (generated)")
    print("  ✓ CODEGEN_DESIGN.md")
    print("  ✓ CODEGEN_VALIDATION.md")
    print()
    print("✨ Deterministic ontology generation code ready!")
    print()
    print("To regenerate ontology:")
    print("  $ python ontology/generate_ontology.py")
    print()
    print("Next: Test the generated ontology, then proceed with Option 1 (Python implementation)")
    print()
...
if __name__ == "__main__":
    try:
        ...
        print("\n\n⚠️  Ontology codegen strand interrupted by user")
        sys.exit(1)
